...
export function useNASPerformanceMonitoring() {
  ...
  const startOperation = useCallback((operationId: string, operationType: string) => {
  ...
  const measureOperation = useCallback(async <T>(
    ...
    try {
      ...
      endOperation(operationId, operationType, { ...metadata, success: true });
      return result;
    } catch (error) {
...
export function useArchitectureEvaluationMonitoring() {
  ...
  const evaluateArchitecture = useCallback(async (
    architecture: any,
    evaluationFunction: () => Promise<any>
  ) => {
    ...
    const result = await measureOperation(operationType, evaluationFunction, metadata);
...
export function useDatabaseMonitoring() {
  ...
  const monitorDatabaseOperation = useCallback(async <T>(
    operation: string,
    table: string,
