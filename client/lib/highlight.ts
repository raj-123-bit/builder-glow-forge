...
export const HIGHLIGHT_PROJECT_ID =
  import.meta.env.VITE_HIGHLIGHT_PROJECT_ID || "demo-project";
...
const consoleFallback = {
  ...
  track: (event: string, data: any) =>
    console.log("[Debug] Track:", event, data),
  consumeError: (error: Error, category: string, data: any) =>
    console.error("[Debug] Error:", category, error, data),
};
...
export const initializeHighlight = async () => {
  ...
  try {
    ...
    if (H && typeof H.init === "function") {
      H.init(HIGHLIGHT_PROJECT_ID, {
        ...
        networkRecording: {
          ...
          urlBlocklist: ["https://api.openai.com", "https://api.anthropic.com"],
        },
    ...
    console.warn(
      "Highlight.io failed to initialize, using console fallback:",
      error,
    );
    return consoleFallback;
  }
