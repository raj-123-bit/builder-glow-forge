...
export default async (request: Request, context: Context) => {
  ...
  try {
    ...
    if (pathname === "/api/debug/log" && request.method === "POST") {
      ...
      console.log(
        `[NAS Debug ${logData.level.toUpperCase()}]`,
    ...
    if (pathname === "/api/debug/performance" && request.method === "POST") {
      ...
      console.log(
        "[NAS Performance]",
        JSON.stringify(enhancedPerfData, null, 2),
      );
    ...
    if (pathname === "/api/debug/error" && request.method === "POST") {
      ...
      if (errorData.level === "error") {
        console.error(
          "[NAS CRITICAL ERROR]",
          JSON.stringify(enhancedErrorData, null, 2),
        );
      } else {
    ...
    if (pathname === "/api/debug/session" && request.method === "POST") {
      ...
      console.log(
        "[NAS Session Analytics]",
        JSON.stringify(enhancedSessionData, null, 2),
      );
